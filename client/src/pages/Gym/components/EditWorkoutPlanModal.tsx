...
const EditWorkoutPlanModal = ({ isOpen, onClose, onSuccess, plan }: EditWorkoutPlanModalProps) => {
    ...
    useEffect(() => {
        if (plan) {
            setFormData({
                ...
                days: plan.days?.length > 0 ? plan.days : [{ dayName: '', exercises: [{ name: '', sets: '', reps: '', weight: '' }] }]
            });
        }
    }, [plan]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(`/api/gym/plans/workout/${plan._id}`, formData);
            setIsSuccess(true);
            setTimeout(() => {
                setIsSuccess(false);
                onSuccess();
            ...
            setError(err.response?.data?.msg || 'Error updating workout plan.');
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-zinc-950/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-0.5">Physical Strategy Update</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-zinc-900/50 border border-white/5 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-[#ffd700] transition-all font-bold text-white"
                                    />
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-zinc-500 ml-1 uppercase tracking-widest leading-none">Intelligence Metadata</label>
                                <input
                                    type="text"
                                    name="description"
                                    value={formData.description}
                                    onChange={handleChange}
                                    className="w-full bg-zinc-900/50 border border-white/5 rounded-2xl py-4 px-4 focus:outline-none focus:border-[#ffd700] transition-all font-bold text-white"
                                />
                            </div>
                        </div>
                        ...
                        <div className="pt-4">
                            <div className="flex items-center justify-between mb-6">
                                <label className="text-[10px] font-black text-zinc-500 ml-1 uppercase tracking-widest leading-none">Operational Phases</label>
                                <button
                                    type="button"
                                    onClick={addDay}
