import { motion } from 'motion/react';
import { Award } from 'lucide-react';

interface JudgeCardProps {
  name: string;
  role: string;
  experience: string;
  credentials: string;
  index: number;
}

export function JudgeCard({ name, role, experience, credentials, index }: JudgeCardProps) {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.9 }}
      whileInView={{ opacity: 1, scale: 1 }}
      transition={{ delay: index * 0.1 }}
      viewport={{ once: true }}
      whileHover={{ y: -10 }}
      className="bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] border-2 border-[#2a2a2a] p-6 rounded-lg hover:border-[#d4af37] transition-all duration-300"
    >
      <div className="flex items-center justify-center w-20 h-20 bg-gradient-to-br from-[#c41e3a] to-[#8b1526] rounded-full mb-4 mx-auto">
        <Award className="w-10 h-10 text-[#d4af37]" />
      </div>
      
      <h3 className="text-center mb-2 text-white">{name}</h3>
      <p className="text-center text-[#d4af37] font-semibold mb-3">{role}</p>
      
      <div className="space-y-2 text-center">
        <p className="text-sm text-[#b0b0b0]">
          <span className="font-semibold text-white">{experience}</span> of experience
        </p>
        <p className="text-xs text-[#808080] leading-relaxed">{credentials}</p>
      </div>
    </motion.div>
  );
}
